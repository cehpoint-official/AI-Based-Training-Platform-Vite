...
const Profile = () => {
    ...
    const handleSubmitApiKey = async (event) => {
        ...
        const postURL = `${serverURL}/api/profile`;
        try {
            const response = await axios.post(postURL, { email, mName, apiKey: newApiKey, uid });
            if (response.data.success) {
                ...
                <div className='flex-1 flex items-center justify-center py-10 flex-col'>
                    <div className="md:w-2/5 w-4/5 m-auto py-4 no-scrollbar ">
                        ...
                        <div className='py-6'>
                            ...
                            <Button onClick={handleChangeApiKey} className='mb-4 w-full dark:bg-white dark:text-black bg-black text-white font-bold rounded-none'>Manage Gemini API Key</Button>
                            ...
                            {showPasswordForm && (
                                <form onSubmit={handleSubmit} className="mb-6">
                                    ...
                                    <Button isProcessing={processing} processingSpinner={<AiOutlineLoading className="h-6 w-6 animate-spin" />} className='items-center justify-center text-center dark:bg-white dark:text-black bg-black text-white font-bold rounded-none w-full' type="submit">Submit</Button>
                                </form>
                            )}
                            ...
                            {showApiKeyForm && (
                                <div>
                                    <div className="mb-6">
                                        <p className='font-bold text-black dark:text-white'>Current API Key:</p>
                                    ...
                                    <form onSubmit={handleSubmitApiKey}>
                                        <div className='mb-6'>
                                            <div className="mb-2 block">
                                                <Label className="font-bold text-black dark:text-white" htmlFor="newApiKey" value="New Gemini API Key" />
                                            </div>
                                            <input value={newApiKey} onChange={(e) => setNewApiKey(e.target.value)} className='focus:ring-black focus:border-black border border-black font-normal bg-white rounded-none block w-full dark:bg-black dark:border-white dark:text-white' id="newApiKey" type="text" />
                                        </div>
                                        <Button isProcessing={processing} processingSpinner={<AiOutlineLoading className="h-6 w-6 animate-spin" />} className='items-center justify-center text-center dark:bg-white dark:text-black bg-black text-white font-bold rounded-none w-full' type="submit">Submit New Key</Button>
                                    </form>
                                </div>
                            )}
                        </div>
